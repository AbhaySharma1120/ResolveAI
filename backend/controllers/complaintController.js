...
export const createComplaint = async (
  ...
  try {
    ...
    const normalizedCampusArea =
      campusArea.trim();

    const normalizedSpecificArea =
      specificArea.trim();
    ...
    const geminiAnalysis =
      await analyzeComplaintWithGemini({
    ...
    const department =
      departmentByCategory[finalCategory];

    const priority =
      geminiAnalysis.suggestedPriority;

    const geminiDuplicate =
      duplicateCandidates.find(
        (candidate) =>
          candidate.trackingId ===
          geminiAnalysis.duplicateTrackingId,
      );

    const fallbackDuplicate =
      duplicateCandidates.find(
        (candidate) =>
          candidate.student?.toString() ===
            req.user._id.toString() &&
          candidate.category ===
            finalCategory &&
          candidate.location?.campusArea ===
            normalizedCampusArea,
      );
    ...
    await Promise.all([
      notifyActiveRole({
        ...
        title:
          createdComplaint.priority ===
          "Critical"
            ? "Urgent complaint submitted"
            : "New complaint submitted",

        text: `${createdComplaint.trackingId} · ${createdComplaint.title}`,
      ...
      notifyActiveRole({
        ...
        path: `/admin/complaints/${createdComplaint.trackingId}`,
      }),
    ]);
...
*/
export const getComplaintByTrackingId =
  async (req, res) => {
    try {
      const trackingId =
...
export const getTriageQueue = async (
  ...
  try {
    ...
    return res.status(500).json({
      ...
      message:
        "Unable to retrieve the triage queue",
    });
  }
};
...
*/
export const assignComplaintToSelf =
  async (req, res) => {
    try {
      const trackingId =
        req.params.trackingId
      ...
      if (!complaint) {
        const existingComplaint =
          await Complaint.findOne({
            ...
            "trackingId status assignedOfficer",
          );
      ...
      await createNotification({
        ...
        text: `${complaint.trackingId} has been assigned to ${req.user.name}.`,
      ...
      return res.status(500).json({
        ...
        message:
          "Unable to assign the complaint",
      });
    }
  };
  ...
  async (req, res) => {
    try {
      ...
      return res.status(500).json({
        ...
        message:
          "Unable to retrieve assigned complaints",
      });
    }
  };
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Status is unchanged and no update message was provided",
          });
      }
      ...
      if (statusChanged) {
        await createNotification({
          ...
          text: `${complaint.trackingId} is now ${status}.`,
...
*/
export const getAllComplaintsForAdmin =
  async (req, res) => {
    try {
      const {
        status,
      ...
      return res.status(500).json({
        ...
        message:
          "Unable to retrieve all complaints",
      });
    }
  };
...
*/
export const updateComplaintByAdmin =
  async (req, res) => {
    try {
      const trackingId =
        req.params.trackingId
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Please select a valid complaint priority",
          });
      }
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "No administrative changes were provided",
          });
      }
      ...
      await createNotification({
        ...
        text: `${complaint.trackingId} · ${changes.join(", ")}.`,
